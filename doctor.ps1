...
function Test-Tool {
    ...
    if (Get-Command $Name -ErrorAction SilentlyContinue) {
        try {
            $version = & $VersionCommand 2>$null | Select-Object -First 1
...
if ($ProjectPath) {
    ...
    Write-Host ""
    Write-Host "Proyecto: $ResolvedProject" -ForegroundColor Cyan

    if (-not (Test-Path $ResolvedProject)) {
        Write-Host "[FAIL] El proyecto no existe." -ForegroundColor Red
        $failed = $true
    }
    else {
        $checks = @(
        ...
        foreach ($check in $checks) {
            if (Test-Path $check.Path) {
                ...
                Write-Host "[WARN] Falta $($check.Name)" -ForegroundColor Yellow
            }
        }
    }
}
...
if ($failed) {
    Write-Host ""
    Write-Host "Diagnóstico finalizado con errores obligatorios." -ForegroundColor Red
    exit 1
}
